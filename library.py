...
def update_fsrecords(path):
    ...
    if os.path.isdir(path):
        for node in os.listdir(path):
            update_fsrecords('/'.join([path, node]))
...
class Song(Item):
    def __init__(self, id, name, track_performer, album, artist, length, bitrate,
        ...
        self.track_performer = track_performer or artist.name
        self.album = album
        self.artist = artist
        self.length = length
    ...
    def check_fs(self):
        return os.path.exists(self.path) \
               and os.path.getsize(self.path) == self.size
...
##        threading.Thread(target=self._load_data).start()
    ...
    def update(id, **kwargs):
        #TODO: ensure track_artist is not set the same as artist.name
...
@route('/library/song/<int:id>/data')
@util.print_headers
def get_song_data(id):
    song = Song.get(id)
    return util.send_file_partial(song.path, song.mime)
